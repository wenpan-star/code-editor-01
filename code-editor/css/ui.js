 ...
 *
 * 【v8.6.1 变更】
 *   beforeunload 处理器增加 EditorState.skipBeforeUnload 检测：
 *   设置导入成功后由 settings-io.js 置位，直接放行刷新，
 *   避免 location.reload() 触发浏览器的原生二次确认框。
 *
 * 【v8.5.5 变更】
...
function bindPageLifecycleEvents() {
    ...
    window.addEventListener('beforeunload', function(event) {
        ...
        emergencySave();
        // v8.6.1：settings-io.js 导入成功后置位 skipBeforeUnload，
        // 明确表示"此次刷新是用户主动发起的"，直接放行，
        // 避免与 confirmOverwriteSettings 的未保存代码确认重复。
